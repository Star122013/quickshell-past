...
Item {
  ...
  signal toggleMediaMenu()
  signal toggleSystemStatusMenu()
  ...
  function workspaceText() {
    if (!root.niri) return "Workspace --"
    if (root.niri.activeWorkspaceId < 0) return "Workspace --"
    return "Workspace " + (root.niri.activeWorkspaceName || root.niri.activeWorkspaceId)
  }
    ...
    Rectangle {
      ...
      Row {
        ...
        Item {
          ...
          Connections {
            target: root.stats
            function onCpuPercentChanged() { cpuRing.requestPaint() }
          }
        ...
        Item {
          ...
          Connections {
            target: root.stats
            function onMemPercentChanged() { memRing.requestPaint() }
          }
        ...
        Item {
          ...
          Connections {
            target: root.stats
            function onDiskPercentChanged() { diskRing.requestPaint() }
          }
      ...
      MouseArea {
        ...
        onClicked: root.toggleSystemStatusMenu()
      }
    }
